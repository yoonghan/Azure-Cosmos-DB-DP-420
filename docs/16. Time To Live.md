...
- TTL **resets** after every modification of record. I.e. if you update a record, the TTL is reseted.
...
![TTL Settings](img/ttl_settings.png)
...
## In Item level
An explicit TTL value on an item (e.g., 10 or -1) always takes precedence over the container's default setting, whether that default is infinite (-1) or a positive number (but NOT if the default is null, which globally disables TTL).
...
5. If using the Azure Cosmos DB for MongoDB API, you can use custom TTL fields. But custom TTL fields are based on epoch time (Unix timestamp) and NOT seconds. 

## Notes
1. Restoration of container that pass the ttl can result immediate deletion. this consumes RU and if to avoid deletion, use --disable-ttl.
