...
## Hierarchy
A **Fleet** can group multiple Accounts for cross-account throughput pooling and centralized analytics. An **Account** is the primary distribution/availability boundary.

```
Fleet (multi-account RU pooling & central analytics)
  -> Account (replication, consistency)
    -> Database (database-level throughput)
      -> Container (container-level throughput)
        -> Item
```
...
-- | -- | -- | --
Fleet Level | NO | NO | NO (Manages multi-account RU pooling & cross-account analytics)
...
> [!NOTE]
> **Status (2026):** Shared throughput (database-level throughput) **is still supported** in Azure Cosmos DB. However, Microsoft generally recommends provisioning throughput at the **container level** for most workloads to prevent "noisy neighbor" issues where one active container consumes the majority of the shared RU budget.
...
## Fleets (Azure Cosmos DB Fleets)

**Azure Cosmos DB Fleets** is a management and capacity solution designed for multi-tenant SaaS applications and enterprise workloads operating across many Cosmos DB accounts.

### Why Use Fleets?
- **Account-per-Tenant Pattern:** Multi-tenant SaaS architectures often isolate each tenant into a dedicated Cosmos DB account for security, compliance, or Customer-Managed Keys (CMK).
- **Cost & Operational Management:** Managing provisioned throughput across hundreds or thousands of separate accounts leads to over-provisioning (idle capacity) and operational overhead.

### Key Features
1. **Fleet Pools (Cross-Account Shared Throughput):**
   - Allows multiple Cosmos DB accounts to share a pooled throughput allocation (RU/s).
   - Accounts maintain a baseline dedicated RU/s for steady-state workloads, but can dynamically draw from the shared Fleet Pool during traffic spikes.
   - Optimizes costs by eliminating the need to over-provision capacity for every individual tenant account.
2. **Fleet Analytics:**
   - Centralizes monitoring, usage metrics, and cost telemetry across all accounts and subscriptions within the fleet.
   - Exports telemetry in open format (**Apache Delta Lake**) to Azure Data Lake Storage Gen2 or Microsoft Fabric for custom reporting via Power BI or Apache Spark.
