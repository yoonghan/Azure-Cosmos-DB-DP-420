...
Cosmosdb backup operator- can only backup db
Cosmosdb restore operator - can only restore from Db, but requires Cosmos Db operator else cannot create new container
Cosmosdb operator - creates account, db and container. But cannot view data.
...
![Backup Periodic](999.img/backup_periodic.png)
...
# Continous Backup vs Periodic

Continous has only
- 7 days (Free)
- 30 days
Continous is only Zone/Local. You can achieve geo backup only and if geo-write is enabled.
