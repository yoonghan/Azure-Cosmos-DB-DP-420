...
2. Use **last writer wins** that is based on timestamp, so no headache.
3. last writer wins with custom field. Field must be in epoch(date time in numeric format)
4. Conflict Feed - Manual (you set customResolutionPolicy to Custom, but empty store procedure). Need to call via a trigger to execute container.Conflicts.GetConflictQueryIterator to find all the conflicts. If this method is not called we will waste storage space.
...
2. Custom
    ...
    - Custom with No StoreProcedure / Manual (aka.Conflict Feed)
