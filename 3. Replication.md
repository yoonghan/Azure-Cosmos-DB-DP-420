...
![Global Distribution](999.img/replication.png)
...
![Auto failover](999.img/replication_auto_failover.png)
...
1. The default conflict resolution policy in Azure Cosmos DB is **Last Write Wins**. This policy uses the timestamp (_ts) to determine which item wrote last. 
2. If multi-region write same data in different region, the earliest will just disappear quietly with LWW conflict resolution.
...
## Custom handling conflicts. 

https://learn.microsoft.com/en-us/azure/cosmos-db/nosql/how-to-manage-conflicts?tabs=dotnetv3%2Capi-async%2Casync%2Cv4async#create-a-custom-conflict-resolution-policy
There are 3 types.
1. Last write and win (with Lastwriter win)
2. Via store procedure. This policy is supported in the API for NoSQL only.
3. Code. This is the only manual way to control, the rest are automated, means after setting it will just run without customization.

4. via custom field for last writer win. default is _ts. filled must epoch format (only numbers)
...
Or via Store Procedure (yes, so strange and it's written in Javascript)
...
//This is the magic part, point to store procedure
        ResolutionProcedure = $"dbs/{databaseName}/colls/{containerName}/sprocs/{sprocName}",
    }
};

//Code create store procedure or if already created skip these line.
...
Or implement Custom Conflict Resolution Policy without Store Procedure
```c#
ContainerProperties properties = new(containerName, partitionKey)
{
    ConflictResolutionPolicy = new ConflictResolutionPolicy()
...
![Replication Multi Region](999.img/replication_multi_region.png)

There is Service Failover
![Service Failover](999.img/replication_service_failover.png)
...
![Local Replication](999.img/replication_local.png)
