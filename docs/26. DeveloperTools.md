...
- CosmosClient has a 3-argument constructor `new CosmosClient(accountEndpoint, credential, cosmosoption);`
- Coding practices
...
1. Use stream not LINQ. Meaning no .toList(), but toFeedIterator()
2. Patching often save more than Replace. But from documentation it says there are no significant difference, as before patching it queries first before replacing.
3. Limit per record is 2MB for NoSQL, for MongoDB is 16MB. Not sure for table, Gremlin and Cassandra.
...
- **PatchItemAsync** as your default for updates. It saves bandwidth and RUs, and prevents "clobbering" data if two users update different fields at the same time. Slightly lower RU consumption due to sending less data.
