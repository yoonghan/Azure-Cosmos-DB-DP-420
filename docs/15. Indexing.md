...
2 Types of indexing mode:
  ...
  - Consistent
    - Means will index while create/upsert/delete, while none doesn't. Good if there are no search or index needed on none, so opt for none first when executing a bulk.
    - id and _ts is always indexed for NoSQL and this cannot be disabled (nor does it show in indexing)
...
```
*Note: The path `/events/[]/{name/?, category/?}/?` is invalid.*
...
2. Wildcard Indexes, Azure Cosmos DB API for MongoDB supports Wildcard Indexes - To make query easier, it's similar to put as includedPaths actually. Limitation:
  - a. No compound
  - b. TTL, unique fields
