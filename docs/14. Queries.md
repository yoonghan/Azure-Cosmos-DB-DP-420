...
1. Queries support both single (') and double (") quotes for strings.
...
1. Cross-partition querying does not happen if there is an exact match on the partition key. E.g. `SELECT * from n where n.age=10 AND n.orderId=10`; in this case, if the partition key is `orderId`, there is no cross-partition query. (Note that it uses `AND` and `=`).
