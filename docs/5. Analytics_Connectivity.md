...
Also learn that it is deprecated(as of 2026) and to move forward using Azure Fabric, but may still appear in exam. [Azure Synapse Link is deprecated](https://learn.microsoft.com/en-us/azure/cosmos-db/configure-synapse-link?context=%2Fazure%2Fcosmos-db%2Fcontext%2Fcontext)
...
![Overview](img/analytics_overview.png)
...
- While TTL can be set at the container or item level, ATTL can only be set at the container level currently.
...
The sink used with Azure Stream Analytics in this context should be the Cosmos DB SQL API.
...
Common Use Cases for Managing ATTL (Analytical TTL):

  - Long-term Storage: Use ATTL = -1 to retain historical data for compliance or trends analysis.
  - Temporary Analysis: Use ATTL = 0 to delete temporary analytical data when testing configurations.
  - Hybrid Workloads: Combine ATTL policies with scaling strategies like autoscale for dynamic workloads.
