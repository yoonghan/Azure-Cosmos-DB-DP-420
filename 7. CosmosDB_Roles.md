...
![Master Key](999.img/rbac_master_key.png)
...
# SDK limitation
- incase tutorial fails without master key as these are no longer available. Might be due to disableKeyBasedMetadataWriteAccess?

If your applications do any of the following actions, they'll no longer be able to do these actions through the Cosmos DB SDK, tools that connect via service principal. These actions will need to be executed from Azure Resource Manager Templates, PowerShell, Azure CLI, REST, or Azure Management Library.
