...
2. Region priorities doesn’t need to be in sequence, can be 2,5,10.
3. Priority 0 is active write location.
5. Once failover happens, it updates in background and old write region changes and remains as secondary permanently.
6. If auto failover is enabled, you cannot change the write region.
7. Auto failover is only available for single write region. For multi-region write, it defeats the purpose of failover since all the regions are active-active.
