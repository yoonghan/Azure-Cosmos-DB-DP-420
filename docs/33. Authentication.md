...
### Master Key
1. You have both primary/secondary and direct use of Key/password.
2. This can be disabled with disableLocalAuth=true.
3. Even portal's explorer does not work, to enable, need to login with RBAC icon.

### Resource token
1. This approach require you to create (via SDK/Code) a CosmosDB user and its complicated.
2. Master key is required to create resource token/permission.
3. NOTE, resource token is not an RBAC, user can actually grant full access "Permission.ALL" as it uses master key. To prevent this, the resource token generator is best to be a middleware (another application), though it complicates things. But using resource token generator via a middleware you can custom control. (Just like SAS in Azure Storage but custom written)
    a. Authentication
    b. Time-based access
4. Resource Token in SDK are short-lived (1 hour)
...
_(Below is created using Gemini - to get a gist, not an accurate diagram)_
...
Steps
**Step 1:** Enable the Managed Identity on Azure Data Factory (ADF)
You don't have to "create" the identity; you just enable it. The recommended type for your scenario is the System-Assigned Managed Identity.
...
- Set the System-Assigned status to On and save.
Azure will automatically register this identity in Azure Active Directory (Azure AD). You'll get an Object ID (Principal ID). 

**Step 2:** Grant the Managed Identity Access to Cosmos DB (Azure RBAC)
This step uses Azure Role-Based Access Control (Azure RBAC) to give the ADF's new identity the specific permissions it needs on the Cosmos DB account.
...
**Step 3:** Configure the Linked Service in ADF
Now you configure the connection in your ADF pipeline to use the identity, not a key.
...
- Enter the Cosmos DB Account Endpoint and Database name.
The ADF runtime will now automatically use the system-assigned managed identity to authenticate using a token. You never entered a key or a secret. Minimal maintenance achieved!
