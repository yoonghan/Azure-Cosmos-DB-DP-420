...
Note: Switching throughput types requires the **MIGRATE** keyword. But consider on the command `az cosmosdb sql database create` there are no parameters to specify the throughput-type.
...
![Autoscale uses max-throughput](img/cli_command_autoscale.png)